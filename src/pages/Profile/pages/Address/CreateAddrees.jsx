...
import { addAddress, editAddress } from "@/store/addressSlice"
import { useEffect } from "react"


const CreateAddrees = ({ isOpen, setIsOpen, addressDetails = null }) => {
    ...
    const editAddressHandler = (data) => {
        dispatch(editAddress({ token, id:addressDetails._id ,data }))
        setIsOpen(false)
        toast.success("با موفقیت ویرایش شد.")
    }

    const createAddressHandler = (data) => {
                    ...
                    w-full rounded-sm text-sm p-2 resize-none outline-0 h-22"
                        ...
                        <div className="text-sm">
                            <PrimaryButton type="submit"
                                text={`${addressDetails ? "ویرایش آدرس" : "ثبت آدرس"}`} />
                        </div>
                    </form>
